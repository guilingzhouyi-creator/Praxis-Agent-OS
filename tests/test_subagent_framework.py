...
class TestSubAgentSpec:
    ...
    def test_spec_defaults(self):
        from services.subagent_framework import SubAgentSpec
...
class TestSpecRegistration:
    ...
    def test_register_spec(self):
        from services.subagent_framework import SubAgentDispatcher, SubAgentSpec
        d = SubAgentDispatcher()
...
class TestResultMerger:
    ...
    def test_merge_empty(self):
        from services.subagent_framework import ResultMerger
...
class TestApiHandlers:
    ...
    def test_handle_subagent_specs(self):
        from services.subagent_framework import handle_subagent_specs
        r = handle_subagent_specs()
        assert r["success"]
        assert r["count"] >= 4

    def test_handle_subagent_dispatch_missing(self):
        from services.subagent_framework import handle_subagent_dispatch
    ...
    def test_handle_subagent_result_missing(self):
        from services.subagent_framework import handle_subagent_result
        r = handle_subagent_result({})
        assert not r["success"]

    def test_handle_subagent_cancel_missing(self):
        from services.subagent_framework import handle_subagent_cancel
        r = handle_subagent_cancel({})
        assert not r["success"]

    def test_handle_subagent_list(self):
        from services.subagent_framework import handle_subagent_list
        r = handle_subagent_list({})
        assert r["success"]

    def test_handle_subagent_spec_register(self):
        from services.subagent_framework import handle_subagent_spec_register
        r = handle_subagent_spec_register({"name": "api-agent", "description": "api test"})
        assert r["success"]

    def test_handle_subagent_merge_empty(self):
        from services.subagent_framework import handle_subagent_merge
