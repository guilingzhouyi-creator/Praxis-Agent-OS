...
class TestToolSpec:
    ...
    def test_auto_gates(self):
        from services.tool_spec import ToolSpec
...
class TestRegister:
    ...
    def setup_method(self):
        from services.tool_spec import clear_tools
        clear_tools()

    def test_register_and_get(self):
        from services.tool_spec import register, ToolSpec, TOOL_REGISTRY, clear_tools
        clear_tools()
    ...
    def test_list_by_category(self):
        from services.tool_spec import register, ToolSpec, list_tools, clear_tools
        clear_tools()
        for i in range(3):
...
class TestExecute:
    ...
    def test_execute_success(self):
        from services.tool_spec import execute_tool, clear_tools
        clear_tools()
        recorded = []
        def handler(args, agent):
            recorded.append(args)
            return {"success": True, "data": "done"}
        from services.tool_spec import register, ToolSpec, ParamSpec
    ...
    def test_execute_no_handler(self):
        from services.tool_spec import execute_tool, register, ToolSpec, clear_tools
        clear_tools()
...
class TestMute:
    ...
    def test_mute_tool(self):
        from services.tool_spec import mute_tool, is_muted, clear_mutes, clear_tools
        clear_mutes()
        clear_tools()
        mute_tool("some_tool")
    ...
    def test_unmute_tool(self):
        from services.tool_spec import mute_tool, unmute_tool, is_muted, clear_mutes, clear_tools
        clear_mutes()
        clear_tools()
    ...
    def test_is_muted_default(self):
        from services.tool_spec import is_muted, clear_mutes
        clear_mutes()
