...
class TestRuleDescriptorConstruction:
    def test_minimal(self):
        rule = RuleDescriptor(id="test.1", section="§1", severity=RuleSeverity.MUST, description="Minimal rule")
    ...
    def test_created_at_is_set(self):
        ...
        before = time.time()
        rule = RuleDescriptor(id="ts", section="§1", severity=RuleSeverity.MAY, description="Timestamp check")
        assert before <= rule.created_at <= time.time() + 1


class TestRuleDescriptorEvaluate:
    def test_no_check_fn_always_pass(self):
        rule = RuleDescriptor(id="noop", section="§1", severity=RuleSeverity.MUST, description="No check")
    ...
    def test_check_fn_returns_pass(self):
        ...
        rule = RuleDescriptor(
            id="pass", section="§1", severity=RuleSeverity.MUST, description="Always pass", check_fn=dummy_fn
        )
    ...
    def test_check_fn_returns_warn(self):
        ...
        rule = RuleDescriptor(
            id="warn", section="§1", severity=RuleSeverity.SHOULD, description="Always warn", check_fn=dummy_fn
        )
    ...
    def test_check_fn_returns_block(self):
        ...
        rule = RuleDescriptor(
            id="block", section="§1", severity=RuleSeverity.MUST, description="Always block", check_fn=dummy_fn
        )
    ...
    def test_check_fn_returns_none_falls_to_pass(self):
        ...
        rule = RuleDescriptor(
            id="none-fn", section="§1", severity=RuleSeverity.MUST, description="None fn", check_fn=dummy_fn
        )
    ...
    def test_check_fn_receives_correct_arguments(self):
        ...
        rule = RuleDescriptor(id="spy", section="§1", severity=RuleSeverity.MUST, description="Spy", check_fn=spy_fn)
    ...
    def test_territory_defaults_to_empty_list(self):
        ...
        rule = RuleDescriptor(
            id="territory-default",
            section="§1",
            severity=RuleSeverity.MUST,
            description="Territory default",
            check_fn=spy_fn,
        )
...
class TestRuleDescriptorToDict:
    ...
    def test_to_dict_no_tags(self):
        rule = RuleDescriptor(id="notags", section="§1", severity=RuleSeverity.MAY, description="No tags")
        d = rule.to_dict()
        assert d["tags"] == []
...
class TestRuleDescriptorImmutability:
    def test_cannot_mutate_fields(self):
        rule = RuleDescriptor(id="immutable", section="§1", severity=RuleSeverity.MUST, description="Immutable test")
        try:
            rule.id = "changed"
    ...
    def test_tags_are_frozenset(self):
        rule = RuleDescriptor(
            id="frozen-tags", section="§1", severity=RuleSeverity.MUST, description="Frozen tags", tags=frozenset({"x"})
        )
