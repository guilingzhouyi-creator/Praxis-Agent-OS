...
class TestCellCacheInject:
    ...
    def test_inject_basic(self):
        ...
        r = cache.inject(
            key="decision:use_poetry",
            value="Use Poetry not pip",
            summary="prefer Poetry",
            agent_id="agent-a",
            entry_type="decision",
        )
    ...
    def test_inject_with_tags(self):
        ...
        r = cache.inject(
            key="test:with_tags", value="data with tags", summary="s", agent_id="agent-b", entry_type="observation"
        )
    ...
    def test_inject_then_lookup(self):
        ...
        cache.inject(key="k1", value="value1", summary="v1", agent_id="agent-c", entry_type="observation")
...
class TestCellCacheLookup:
    ...
    def test_lookup_after_expiry(self):
        ...
        cache.inject(
            key="exp_key", value="data", summary="s", agent_id="agent-d", entry_type="observation", ttl=0
        )  # 0 TTL → immediate expiry
...
class TestCellCachePromote:
    ...
    def test_promote_does_not_crash(self):
        ...
        r = cache.promote(key="promoted_key", summary="promoted", value="promoted value", location="l3", importance=0.7)
...
class TestCellCacheFlush:
    ...
    def test_flush_returns_count(self):
        ...
        cache.inject(key="f1", value="v1", summary="s1", agent_id="agent-f", entry_type="observation")
        n = cache.flush()
...
class TestCellCacheEvict:
    ...
    def test_evict_index_on_overflow(self):
        ...
        for i in range(50):
            cache.inject(
                key=f"evict:{i}", value=f"data{i}", summary=f"s{i}", agent_id="agent-e", entry_type="observation"
            )
        # Should not crash
        stats = cache.stats()
