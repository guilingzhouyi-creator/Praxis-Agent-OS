...
class TestToolPolicy:
    ...
    def test_add_rule(self):
        ...
        rule = PolicyRule(action=PolicyAction.ENABLE, tool="read_file", scope=PolicyScope.ROLE, scope_id="reader")
        ToolPolicy.add(rule)
    ...
    def test_add_rule_then_clear(self):
        ...
        rule = PolicyRule(action=PolicyAction.ENABLE, tool="tool_x", scope=PolicyScope.GLOBAL, scope_id="*")
        ToolPolicy.add(rule)
        ToolPolicy.clear()
    ...
    def test_add_and_remove_rule(self):
        ...
        rule = PolicyRule(action=PolicyAction.ENABLE, tool="tmp_tool", scope=PolicyScope.GLOBAL, scope_id="*")
        ToolPolicy.add(rule)
    ...
    def test_requires_approval_with_rule(self):
        ...
        rule = PolicyRule(
            action=PolicyAction.REQUIRE_APPROVAL, tool="risky_tool", scope=PolicyScope.GLOBAL, scope_id="*"
        )
        ToolPolicy.add(rule)
