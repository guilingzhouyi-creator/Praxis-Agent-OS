...
class TestTimeScheduler:
    def test_init(self):
        from l3.scheduler_time import get_time_scheduler, reset_time_scheduler
...
class TestL3Router:
    def test_init(self):
        from l3.scheduler_router import L3Router
        router = L3Router()
...
class TestRequestPool:
    def test_init(self):
        from l3.scheduler_router import RequestPool
        pool = RequestPool(capacity=8)
        assert pool is not None

    def test_enqueue_dequeue(self):
        from l3.scheduler_router import RequestPool
        from l3.scheduler_types import Task
        pool = RequestPool(capacity=8)
...
class TestSchedulerTypes:
    ...
    def test_agent_info(self):
        from l3.scheduler_types import AgentInfo
        info = AgentInfo(id="a1", territory=["."])
        assert info.id == "a1"

    def test_time_slice(self):
        from l3.scheduler_types import TimeSlice
