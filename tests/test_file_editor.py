...
class TestUndoRedo:
    ...
    def test_undo_nothing(self):
        from services.file_editor import EditEngine
...
class TestPatchSystem:
    ...
    def test_patch_create_and_apply(self):
        from services.file_editor import EditEngine, DiffEdit, PatchManager
    ...
    def test_patch_list(self):
        from services.file_editor import EditEngine, PatchManager
        engine = EditEngine()
        mgr = PatchManager(engine)
        pr = mgr.list_patches()
        assert pr["success"]
...
class TestApiHandlers:
    ...
    def test_handle_fs_edit_basic(self):
        from services.file_editor import handle_fs_edit
    ...
    def test_handle_fs_history(self):
        from services.file_editor import handle_fs_history

        r = handle_fs_history({"limit": 5})
        assert r["success"]

    def test_handle_fs_undo_redo(self):
        from services.file_editor import handle_fs_undo, handle_fs_redo, get_engine
        # Ensure clean state
        eng = get_engine()
        while eng._history:
            eng._history.pop()
