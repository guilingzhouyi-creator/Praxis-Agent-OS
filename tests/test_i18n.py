...
class TestI18nCore:
    ...
    def test_set_locale(self):
        from services.i18n import get_locale, set_locale
        prev = get_locale()
        set_locale("zh-CN")
    ...
    def test_t_zh_cn(self):
        from services.i18n import t, set_locale, get_locale
        prev = get_locale()
        set_locale("zh-CN")
        result = t("shell.command.help")
        assert "显示" in result
    ...
    def test_get_available_locales(self):
        from services.i18n import get_available_locales
...
class TestI18nL2Shell:
    def test_list_commands_localized_en(self):
        from services.l2_shell import list_commands, reset_state
        reset_state()
        cmds = list_commands()
...
class TestI18nKernelErrors:
    def test_set_locale_delegates_to_i18n(self):
        from kernel.errors import set_locale, get_locale
        prev = get_locale()
        set_locale("zh-CN")
        from services.i18n import get_locale as _i18n_get
        assert _i18n_get() == "zh-CN"
        set_locale(prev)

    def test_error_to_dict(self):
        from kernel.errors import PraxisError
...
class TestI18nToolSpec:
    def test_list_tools_with_locale(self):
        ...
        from services.tool_spec import list_tools, TOOL_REGISTRY
