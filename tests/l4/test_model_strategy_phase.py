...
class TestPhaseStrategy:
    def test_card_phase_strategy_field(self):
        ...
        phase = card.add_phase(name="plan", mode=PhaseMode.SINGLE, strategy="deep")
        ...
        r = cardwrite_handler(
            {
                "nature": "execution",
                "title": "t",
                "phases": [{"name": "plan", "strategy": "deep", "tasks": [{"action": "read_file", "target": "x.py"}]}],
            }
        )
...
class TestSubagentStrategy:
    ...
    def test_resolve_applies_strategy(self):
        ...
        _setup()
        spec = SubAgentSpec(name="deep-runner", description="d", model_spec="subagent", strategy="deep")
