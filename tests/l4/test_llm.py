...
class TestLlmEngine:
    ...
    def test_engine_tool_use_basic(self):
        ...
        r = engine.tool_use("use test_tool", tools=tools, system="test", max_turns=2)
        assert isinstance(r, dict)
...
class TestToolUse:
    ...
    def test_tool_use_with_args(self):
        ...
        r = engine.tool_use("process hello", tools=tools, system="test", max_turns=2)
        assert isinstance(r, dict)
...
class TestHooks:
    ...
    def test_counter_hook_integration(self):
        ...
        reset_engine()
        engine = get_engine()
        r = engine.generate("counter-test", system="test", max_tokens=64, user_id="test-user")
        assert isinstance(r, dict)
...
class TestRetryConfig:
    ...
    def test_backoff_base(self):
        from l1.kernel.params.api import LLM_TRANSIENT_BACKOFF_BASE

        assert LLM_TRANSIENT_BACKOFF_BASE == 3

    def test_empty_response_waits(self):
        from l1.kernel.params.api import LLM_EMPTY_RESPONSE_WAITS
