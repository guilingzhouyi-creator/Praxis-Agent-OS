...
class TestMemoryInit:
    def test_snapshot_path_format(self):
        from services.memory_init import _snapshot_path
        path = _snapshot_path("test")
        assert "test" in path
        assert path.endswith(".json")

    def test_read_write_json(self):
        from services.memory_init import _read_json, _write_json
        td = tempfile.mkdtemp()
    ...
    def test_save_boot_snapshot(self):
        from services.memory_init import save_boot_snapshot
    ...
    def test_latest_snapshot(self):
        from services.memory_init import _latest_snapshot
        r = _latest_snapshot()
