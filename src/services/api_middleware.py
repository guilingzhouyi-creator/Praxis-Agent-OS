...
class RequestLogMiddleware(Middleware):
    ...
    def process_response(self, response: Response) -> Response:
        elapsed = time.time() - getattr(Response, "_start_time", time.time())
        # Access the original request's start time via a closure or stored ref
        # Since we don't store request on response, use a best-effort log
        logger.info("API %s → %s (%.0fms)",
                     getattr(self, "_last_method", "?"),
                     response.status,
                     elapsed * 1000)
        return response
