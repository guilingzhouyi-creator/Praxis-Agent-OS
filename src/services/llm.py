...
Usage:
  from services.llm import think, analyze
...
class LLMEngine:
    ...
    def _get_strategy(self):
        from .cache_strategy import get_strategy
    ...
    def _build_provider(self) -> LLMProvider:
        ...
        cls = _PROVIDER_REGISTRY.get(p)
        if cls is not None:
            if p == "ollama":
                return cls(self.config.api_url, self.config.model)
            if p == "openai":
                return cls(self.config.api_key, self.config.api_url, self.config.model)
            if p == "anthropic":
                return cls(self.config.api_key, self.config.api_url, self.config.model, self.config.cache_breakpoints)
            if p == "websocket":
                return cls(self.config.api_url, self.config.model, self.config.api_key)
            # Generic provider: pass config kwargs and let __init__ sort them
            try:
                return cls(
...
try:
    ...
    _prov_mod = _il.import_module("services.llm_providers")
