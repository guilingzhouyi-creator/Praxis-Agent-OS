...
def boot(agent_config: list[tuple[str, str, list[str]]] | None = None,
    ...
    try:
        from kernel.persist import restore
        r = restore()
        if r.get("success"):
    ...
    if PERSIST_AUTO:
        def _save_loop():
            while True:
                ...
                try:
                    from kernel.persist import save
                    save()
                except Exception as e:
...
def _init_skills_and_network() -> dict:
    ...
    try:
        from kernel.net import get_net; get_net().start(); results["network"] = "ok"
...
def _create_cell(agent_config: list[tuple[str, str, list[str]]] | None = None) -> dict:
    ...
    from kernel import register_process
    from kernel.vfs import get_vfs, MountType
