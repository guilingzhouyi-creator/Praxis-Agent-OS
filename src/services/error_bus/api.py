...
def handle_log_errors(body: dict | None = None) -> dict:
    ...
    bus = get_bus()
    return bus.query(
        level=b.get("level"), error_code=b.get("error_code"),
        component=b.get("component"), service=b.get("service"),
        agent_id=b.get("agent_id"),
        since=_parse_float(b, "since"), until=_parse_float(b, "until"),
        offset=_parse_int(b, "offset", 0), limit=_parse_int(b, "limit", 50))
