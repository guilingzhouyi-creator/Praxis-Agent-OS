...
class ApiGateway(ApiHandlers):
    ...
    def _register_defaults(self) -> None:
        ...
        try:
            from services.sse_bridge import ensure_active
            ensure_active()
        except Exception as e:
    ...
    def _run(self) -> None:
        ...
        class _Handler(http.server.BaseHTTPRequestHandler):
            ...
            def _handle_via_middleware(self, method: str) -> None:
                """Route request through MiddlewareChain."""
                ...
                req = self._build_request(method, self.path)
                # Inject route params
                handler, params = self.gateway._match_route(method, path)
                req.params = params

                def route_handler(r: Request) -> dict:
                    # Build body dict: merge request body with route params
                    data = dict(r.body)
                    if params.get("id"):
                        data["_id"] = params["id"]
            ...
            def _do_sse(self) -> None:
                ...
                try:
                    ...
                    try:
                        from services.sse_bridge import unsubscribe as _unsub
            ...
            def do_POST(self):
                ...
                req = self._build_request("POST", self.path, body=body)
                handler, params = self.gateway._match_route("POST", req.path)
                req.params = params
            ...
            def do_DELETE(self):
                body = self._read_body()
                req = self._build_request("DELETE", self.path, body=body)
                handler, params = self.gateway._match_route("DELETE", req.path)
                req.params = params
