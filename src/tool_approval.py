...
def request_ring3_approval(tool_name: str, agent_id: str,
    ...
    return {"approved": False, "witness": WitnessStatus.PENDING,
            "review_id": review_id, "status": WitnessStatus.AWAITING}
...
def check_ring3_witness(review_id: str, agent_id: str) -> dict[str, Any]:
    ...
    for msg in msgs:
        if (msg.msg_type == MessageType.CROSS_REVIEW_RESP
                and msg.reply_to == review_id):
            return {"approved": msg.payload.get("approved", False),
                    "witness": msg.sender,
                    "comments": msg.payload.get("comments", ""),
                    "review_id": review_id}
    return {"approved": False, "witness": WitnessStatus.PENDING,
            "review_id": review_id, "status": WitnessStatus.STILL_WAITING}
