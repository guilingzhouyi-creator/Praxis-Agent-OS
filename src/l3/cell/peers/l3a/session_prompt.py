...
class SessionPromptMixin:
    ...
    def prompt(self, text: str, mode: str = "steer") -> dict:
        ...
        try:
            ...
            sc.ingest(
                _Mp(
                    name="l3a.tokens.projected",
                    value=float(pre_tokens),
                    tags={"session": self.id, "agent": _p.AGENT_ID},
                    timestamp=ts,
                    metric_type="gauge",
                )
            ...
            sc.ingest(
                _Mp(
                    name="l3a.tokens.actual",
                    value=float(post_tokens),
                    tags={"session": self.id, "agent": _p.AGENT_ID},
                    timestamp=ts,
                    metric_type="gauge",
                )
            ...
            if token_saved > 0:
                sc.ingest(
                    _Mp(
                        name="l3a.tokens.saved",
                        value=float(token_saved),
    ...
    def context_stats(self) -> dict:
        ...
        history_tok = _est_tokens(" ".join(m.content for m in self.history._messages)) if self.history._messages else 0
    ...
    def _query_context_window(self) -> int:
        ...
        try:
            ...
            raw = engine.context_window(cell_id=self._cell_id, agent_id=_p.AGENT_ID)
    ...
    def _report_stats(self) -> None:
        ...
        try:
            ...
            sc.ingest(
                MetricPoint(
                    name="l3a.session.turns",
                    value=float(self.turn_count),
                    tags={"cell": self._cell_id, "agent": _p.AGENT_ID, "session": self.id},
                    timestamp=ts,
                    metric_type="gauge",
                )
            ...
            sc.ingest(
                MetricPoint(
                    name="l3a.session.tokens_consumed",
                    value=float(est + stats["epoch_baseline_tokens"]),
                    tags={"cell": self._cell_id, "agent": _p.AGENT_ID, "session": self.id},
                    timestamp=ts,
                    metric_type="counter",
                )
