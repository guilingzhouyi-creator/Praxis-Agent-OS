...
def _get_db() -> sqlite3.Connection:
    ...
    for col in ("title TEXT DEFAULT ''", "ttl REAL DEFAULT 0", "ref_code TEXT DEFAULT ''"):
        try:
...
def _purge_expired() -> int:
    ...
    try:
        conn = _get_db()
        now = time.time()
        cur = conn.execute("DELETE FROM archive WHERE ttl > 0 AND created_at + ttl < ?", (now,))
        conn.commit()
        if cur.rowcount:
...
def _next_ref_code(conn: sqlite3.Connection, fonds: str, series: str) -> str:
    ...
    row = conn.execute("SELECT COUNT(*) FROM archive WHERE fonds = ? AND series = ?", (fonds, series)).fetchone()
