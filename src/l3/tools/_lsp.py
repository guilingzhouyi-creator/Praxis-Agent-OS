...
Wraps l4.lsp.lsp_manager.LspManager for agent-accessible code analysis.
All operations are read-only (Ring 1).
"""
...
def find_references(args: dict, agent_id: str) -> dict:
    ...
    try:
        from l4.lsp.lsp import get_lsp
        lsp = get_lsp()
        result = lsp.find_references(path, line=int(line), column=int(column))
