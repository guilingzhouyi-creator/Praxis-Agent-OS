...
from l3.cell_decompose import auto_agent_map as _auto_agent_map
...
def execute_card(
    ...
    """
    from l3.issue import IssueCard as _IssueCard
    if isinstance(card, _IssueCard):
        return cell.convene(card, agent_map)

    cell._current_user_id = user_id
    try:
        from l3.scheduler import get_scheduler as get_sched
        sched = get_sched()
    ...
    if domain and agent_map is None:
        from l3.cell_decompose import decompose_card as _dc
...
def _raw_to_card(cell, raw_intent: str, domain: str, skip_htn: bool = False):
    ...
    from l3.card_builder import build_card as _build_structured_card
