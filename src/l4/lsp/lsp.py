...
Agents can call LSP tools instead of relying on grep/read:
  - go_to_definition: where a symbol at a position is defined
  - find_references: all usages of a symbol at a position
  - hover_info: type/doc info for a symbol at a position
...
class LocalAnalyzer:
    ...
    def _check_pyright(self) -> bool:
        try:
            r = subprocess.run(
                ["pyright", "--version"], capture_output=True, text=True, timeout=SUBPROCESS_SHORT_TIMEOUT
            )
    ...
    def _walk_python(self, file_path: str = "") -> list[tuple[str, ast.AST]]:
        ...
        results = []
        if file_path:
            full = os.path.join(self.root, file_path)
            if os.path.isfile(full):
                rel = os.path.relpath(full, self.root)
                tree = self._parse(full)
                return [(rel, tree)] if tree else []
            search_dir = full
        else:
    ...
    def symbol_search(self, query: str, file_path: str = "", limit: int = TOOL_LSP_SYMBOL_LIMIT) -> list[Symbol]:
        ...
        for rel, tree in self._walk_python(file_path):
            for node in ast.walk(tree):
                ...
                if sym and (query == "*" or query.lower() in sym.name.lower() or re.search(query, sym.name, re.I)):
                    results.append(sym)
    ...
    def hover_info(self, name: str, file_path: str = "") -> dict:
        ...
        return {
            ...
            "file": sym.file,
            "line": sym.line,
            "docstring": sym.docstring[:LOG_TRUNC_300] if sym.docstring else "",
            "parent": sym.parent,
            "found": True,
        }
    ...
    def diagnostics(self, file_path: str = "") -> list[dict]:
        ...
        if self._pyright_ok:
            ...
            try:
                r = subprocess.run(
                    ["pyright", full, "--outputjson"], capture_output=True, text=True, timeout=LSP_DIAG_TIMEOUT
                )
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef) and not node.returns and node.name != "__init__":
                issues.append(
                    {
                        "file": file_path,
                        "line": node.lineno,
    ...
    def _node_to_symbol(self, node: ast.AST, file_rel: str) -> Symbol | None:
        if isinstance(node, ast.ClassDef):
            ...
            return Symbol(name=node.name, kind="class", file=file_rel, line=node.lineno, docstring=doc)
