...
def _l3a_messages(args: dict) -> dict:
    ...
    if not s:
        return {"success": False, "error": f"session not found: {sid}"}
    page = s.messages(cursor=args.get("cursor") or None, limit=int(args.get("limit", 20)))
    return {"success": True, "data": page.items, "cursor": page.cursor, "total": page.total}
...
def _l3a_resume(args: dict) -> dict:
    ...
    d = get_daemon()
    s = Session.resume_from_archive(archived_id, model_config=d.model_config, registry=d.registry)
    if not s:
        return {"success": False, "error": f"archived session not found: {archived_id}"}
...
_L3A_TOOLS: dict[str, dict[str, Any]] = {
    "l3a_create": {
        ...
        "inputSchema": _schema({"title": _str_prop("Optional session title")}, []),
        "handler": _l3a_create,
    },
    "l3a_prompt": {
    ...
    "l3a_info": {
        ...
        "inputSchema": _schema({"session_id": _str_prop("Session ID")}, ["session_id"]),
        "handler": _l3a_info,
    },
    "l3a_list": {
    ...
    "l3a_peek": {
        ...
        "inputSchema": _schema({"task_id": _str_prop("Task ID from l3a_spawn")}, ["task_id"]),
        "handler": _l3a_peek,
    },
    "l3a_tasks": {
    ...
    "l3a_resume": {
        ...
        "inputSchema": _schema(
            {"archived_session_id": _str_prop("Archived session ID from l3a_list")}, ["archived_session_id"]
        ),
        "handler": _l3a_resume,
    },
    "l3a_compress": {
        ...
        "inputSchema": _schema(
            {
                "session_id": _str_prop("Session ID"),
                "keep_last": {"type": "integer", "description": "Messages to keep (default 10)"},
            },
            ["session_id"],
        ),
    ...
    "l3a_memory": {
        "description": "Report R1-R3 ring usage + ingress rate for a session (or all scopes if session_id omitted).",
        "inputSchema": _schema(
            {
                "session_id": _str_prop("Session ID (omit for all scopes)"),
                "window": {"type": "number", "description": "Ingress window seconds (default 3600)"},
            },
            [],
        ),
        "handler": _l3a_memory,
...
def _schema_from_params(spec: Any) -> dict:
    try:
        ...
        if params:
            ...
            required = []
            for p in params:
                props[p.name] = {"type": _js_type(p.type), "description": getattr(p, "description", "")}
...
def _selected_tools() -> dict[str, dict]:
    return {name: {"description": t["description"], "inputSchema": t["inputSchema"]} for name, t in _L3A_TOOLS.items()}
