...
def create_interactive_shell(cwd: str = "") -> _subprocess.Popen:
    ...
    if cwd:
        kwargs["cwd"] = cwd
    return _subprocess.Popen(cmd, stdin=_subprocess.PIPE, stdout=_subprocess.PIPE, stderr=_subprocess.STDOUT, **kwargs)
...
def tail_file(path: str, n_lines: int = 10) -> list[str]:
    ...
    if not IS_WINDOWS and which("tail"):
        try:
            r = _subprocess.run(
                ["tail", "-n", str(n_lines), path], capture_output=True, text=True, timeout=SUBPROCESS_SHORT_TIMEOUT
            )
            if r.returncode == 0:
