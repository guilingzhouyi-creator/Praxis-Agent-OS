...
def _cmd_disconnect(args: list[str]) -> dict:
    ...
    state = get_state()
    if not state.is_direct(): return {"success": False, "error": "not connected"}
    try:
...
def _cmd_mode(args: list[str]) -> dict:
    ...
    if args and args[0].upper() == "DIRECT":
        ...
        state.switch_to_direct(state.cell_id, state.agent_id); return {"success": True, "mode": "DIRECT"}
    state.switch_to_l3a(); return {"success": True, "mode": "L3A"}
